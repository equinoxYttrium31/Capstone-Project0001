  ...
  useEffect(() => {
    const fetchUsersAndCellGroups = async () => {
        try {
            ...
            // Filter guests
            const filterGuests = usersResponse.data.filter((guest) => 
            ...
            // Count genders
            const maleCount = filteredMembers.filter(user => user.gender === 'Male').length;
            const femaleCount = filteredMembers.filter(user => user.gender === 'Female').length;
            
            // Calculate percentage of male users
            const malePercentage = ((maleCount / filteredMembers.length) * 100).toFixed(2); // Rounded to 2 decimal places
            setMaleProgress(malePercentage); // Set male percentage in progress

            const femalePercentage = ((femaleCount / filteredMembers.length) * 100).toFixed(2); // Rounded to 2 decimal places
