...
const handleLogout = async () => {
    try {
      ...
      document.body.style.overflow = 'auto';
        ...
        toast.error('Error during logout: ' + error.response?.data?.error || 'An unexpected error occurred.');
    }
};
      ...
      <div className="setting_content_container">
        ...
        <div className="settings_selection_container">
          <ul className="setting_Selection">
            <li className={`setting_Selection_list ${isActive === 'PaS' ? 'isActive' : ''}`}
              onClick={() => handleSettingContentClick('PaS')}>Password and Account</li> 
          </ul>
          <ul className="setting_Selection">
            <li className={`setting_Selection_list ${isActive === 'Appearance' ? 'isActive' : ''}`}
              onClick={() => handleSettingContentClick('Appearance')}>Appearance</li> 
          </ul>
        </div>
      </div>
      ...
      {isActive === 'PaS' && (
        <div className="account_setting_modal">
          <div className="PaS_Setting_cont">
            ...
            <p className="PaS_Context">Your password is crucial for protecting your account. Ensure it is strong and unique.</p>
          </div>
            <div className="logout_button_container">
              ...
              <div className="button_container_settings">
                ...
                <button onClick={ handleLogout } className="btn-logout_settings">
                    <img src={logout_ic} alt='logout_ic' className='button_ic_logout'/>
                    Log Out
                </button>
              </div>
            </div>
              ...
              <div className="big-container-Password">
                ...
                <div>
                  <input 
                    ...
                    required 
                  />
                  <label className='Label_Form_Settings ConfPass_Label'>Confirm New Password</label>
                </div>
              </div>
              {error && <p className="error-message">{error}</p>}
              <div className="button_change_cont">
                  <button type="submit" className="btn_change_Pass">Change Password</button>
              </div>
            </form>
          </div>
        </div>
